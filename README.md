...
`continuum.stream-dashboard` is an operator dashboard for Continuum stream activity. It serves an embedded React SPA that surfaces active sessions, playback history, a world map view, and a globe view of where streams are originating, backed by a plugin-owned Postgres schema that mirrors data from the Continuum host database.

## Category

Lives under **Operations** alongside [`continuum-plugin-notifications`](https://github.com/RXWatcher/continuum-plugin-notifications) (the cross-plugin event hub) and [`continuum-plugin-support`](https://github.com/RXWatcher/continuum-plugin-support) (the customer-support shell).
...
| `scheduled_task.v1` | `sync-playback-history` | Copies playback history from the Continuum source database into the plugin schema and applies the configured retention policy. |

## Dependencies

- Continuum plugin host (gRPC plugin runtime from [`continuum-plugin-sdk`](https://github.com/ContinuumApp/continuum-plugin-sdk)).
- Read-only access to the Continuum host's Postgres database (active sessions, playback history). The plugin never writes back to the Continuum schema; it only reads.
...
- **Postgres** (required, two DSNs):
  ...
  - `continuum_database` — read-only DSN for Continuum's public schema. Used to pull active sessions and playback history.
...
- **Playback history** — paginated history rows synced from the Continuum host, filtered by the active retention policy.
...
| `continuum_database.database_url` | yes | Read-only Postgres DSN for the Continuum host's public schema. |
...
The `sync-playback-history` scheduled task pulls new rows from the Continuum source DB into the plugin schema, then applies `RetentionPolicy{Days, MaxRows, MinWatchSeconds, CompletedOnly}` to prune anything outside the policy. The dashboard reports per-run `synced_rows` and `pruned_rows` so retention behavior is observable from the SPA.
